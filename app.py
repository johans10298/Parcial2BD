...
def handler(event,context):
    ...
    s3.upload_file("/tmp/archivo.txt","parcial2-1",url)
    s3.upload_file("/tmp/archivo1.txt","parcial2-1",url1)
...
def scrap(event,context):
    ...
    url = "Headlines/Raw/periodico=El Tiempo""/year="+str(year)+"/month="+str(month)+"/day="+str(day)+"/tiempo.txt"
    url1 = "Headlines/Raw/periodico=El Espectador""/year="+str(year)+"/month="+str(month)+"/day="+str(day)+"/espectador.txt"
    response1 = s3.get_object(Bucket="parcial2-1",Key=url)
    data1 = response1['Body'].read()
    print("hola", data1)
    response = s3.get_object(Bucket="parcial2-1",Key=url)
    ...
    s3.upload_file("/tmp/tiempo.csv","parcial2-2",url11)
